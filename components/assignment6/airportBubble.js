...
function AirportBubble(props) {
    ...
    if (selectedAirline) {
        ...
        const scale = scaleLinear([cities[0].Count, cities[cities.length - 1].Count], [2, width * 0.15]);
        ...
        //  Please set `.tick(200)`. 
        const x = forceX(width / 2.0).strength(0.02);
        const y = forceY(height / 2.0).strength(0.02);
        const collide = forceCollide().radius(node => {
        ...
        const circles = cities.map((d, idx) => {
            if (idx < cities.length - 5) {
                return <circle key={idx} cx={d.x} cy={d.y} r={scale(d.Count)} fill={"#2a5599"} stroke={"black"} strokeWidth={"2"}>
                </circle>
            } else {
                return <>
                    <circle key={idx} cx={d.x} cy={d.y} r={scale(d.Count)} fill={"#ADD8E6"}>
                    </circle >

                    <text key={`text-{idx}`} x={d.x} y={d.y} style={{
                        textAnchor: "middle", stroke: "pink", strokeWidth: "0.5em",
                        fill: "#992a2a", fontSize: 16, fontFamily: "cursive",
                        paintOrder: "stroke", strokeLinejoin: "round"
                    }}>
                        {d.City}
